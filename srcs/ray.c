...
double		set_ray(t_ray *ray, double distance, double wall)
{
...
static void	cast_horizontal(t_game *game, t_ray *ray, double *distance, double *wall)
{
	t_point		inter;
	t_point		step;

	inter.y = (int)floor(game->p.pos.y / CUBE_SIZE) * CUBE_SIZE + (ray->angle < 180 ? -0.001 : CUBE_SIZE);
	inter.x = game->p.pos.x + ((game->p.pos.y - inter.y) * -1) / tan(to_radians(ray->angle));
...
static void	cast_vertical(t_game *game, t_ray *ray, double *distance, double *wall)
{
	t_point		inter;
	t_point		step;

	inter.x = floor(game->p.pos.x / CUBE_SIZE) * CUBE_SIZE + (ray->angle < 270 && ray->angle > 90 ? CUBE_SIZE : -0.001);
	inter.y = game->p.pos.y + ((game->p.pos.x - inter.x) * -1) * tan(to_radians(ray->angle));
...
double		cast(t_game *game, t_ray *ray)
{
	...
	if ((distance.x == -1 && distance.y != -1) || (distance.y != -1 && distance.y <= distance.x))
	{
		ray->face = (ray->angle < 270 && ray->angle > 90 ? FACE_EAST : FACE_WEST);
		return (set_ray(ray, distance.y, wall.y));
	}
	if ((distance.y == -1 && distance.x != -1) || (distance.x != -1 && distance.x < distance.y))
	{
